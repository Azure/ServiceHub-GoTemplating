...
var _ = Describe("PopulateMethods", func() {
	...
	useRemoteProto := false
	remoteProtoConfig := RemoteProtoConfig{}
	...
	It("No change to method files or states files when proto not modified.", func() {
		...
		err = PopulateAllMethodFiles(svcPath, protoPath, deleteGarbageFiles, useRemoteProto, remoteProtoConfig)
		Expect(err).ToNot(HaveOccurred())
	...
	It("Api proto file path doesn't exist", func() {
		err := PopulateAllMethodFiles(svcPath, "incorrectProtoPath.proto", deleteGarbageFiles, useRemoteProto, remoteProtoConfig)
		Expect(err).To(HaveOccurred())
	})
	It("Service path directory doesn't exist", func() {
		err := PopulateAllMethodFiles("incorrectSvcPath", protoPath, deleteGarbageFiles, useRemoteProto, remoteProtoConfig)
		Expect(err).To(HaveOccurred())
	})
	It("should parse remote methods correctly", func() {
		// Mock the getRemoteProtoFileContent function
		originalFunc := getRemoteProtoFileContent
		getRemoteProtoFileContent = func(protoPath string, config RemoteProtoConfig) (string, error) {
			return `
                syntax = "proto3";
                service TestService {
		...
		defer func() { getRemoteProtoFileContent = originalFunc }() // Restore the original function after the test

		methods, err := parseMethods("path/to/proto", true, RemoteProtoConfig{})
		Expect(err).NotTo(HaveOccurred())
		Expect(methods).To(HaveLen(1))
		Expect(methods[0].Name).To(Equal("SayHello"))
	})
})
