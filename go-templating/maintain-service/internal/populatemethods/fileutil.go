...
type RemoteProtoConfig struct {
	Owner string
	Repo  string
}

var getRemoteProtoFileContent = getGithubProtoFileContent // this is a variable so that it can be mocked in tests

func PopulateAllMethodFiles(svcDirName string, protoFilePath string, deleteGarbageFiles bool, useRemoteProto bool, RemoteProtoConfig RemoteProtoConfig) error {
	if !useRemoteProto {
	...
	methodsList, err := parseMethods(protoFilePath, useRemoteProto, RemoteProtoConfig)
	if err != nil {
...
func parseMethods(protoPath string, useRemoteProto bool, RemoteProtoConfig RemoteProtoConfig) ([]*proto.RPC, error) {
	var methodsList []*proto.RPC
	var parser *proto.Parser
	if useRemoteProto {
		protoContent, err := getRemoteProtoFileContent(protoPath, RemoteProtoConfig)
		if err != nil {
			log.Printf("Error getting remote proto file content: %v", err)
			return nil, err
		}
...
func getGithubProtoFileContent(protoPath string, RemoteProtoConfig RemoteProtoConfig) (string, error) {
	...
	fileContent, _, _, err := client.Repositories.GetContents(ctx, RemoteProtoConfig.Owner, RemoteProtoConfig.Repo, protoPath, nil)
