...
func GenerateAllResourceFiles(svcDirName string, svcOutputFileName string) error {
	...
	if (len(svcTmplData) != 1) || (svcTmplData[0].SectionHeader != "Default") {
		...
		// 0666 grants read and write access but not execute
		err = templateutil.CreateFileFromTemplate(outputFilePath, svcTmplFilePath, svcTmplData, []string{"<<", ">>"}, 0666)
		if err != nil {
...
func createDeploymentFile(inputFilePath string, destPath string, tmplFilePath string) error {
	...
	// Allows read and write access but not execute
	err = templateutil.CreateFileFromTemplate(destPath, tmplFilePath, inputData, []string{"<<", ">>"}, 0666)
	if err != nil {
